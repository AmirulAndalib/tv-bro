...
class FaviconExtractor {
    ...
    private val attributePattern: Pattern = Pattern.compile("(\\S+)=['\"]([^>]*?)['\"]")
     ...
     */
    fun extractFavIconsFromURL(url: URL): List<IconInfo> {
        ...
        var hasDefaultIcon = false
        for (ii in result) {
            if (ii.src == DEFAULT_ICON_SRC) {
                hasDefaultIcon = true
                break
            }
        }
